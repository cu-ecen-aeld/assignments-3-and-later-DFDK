...
OUTDIR=/tmp/aeld
KERNEL_REPO=https://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git #git://git.kernel.org/pub/scm/linux/kernel/git/stable/linux-stable.git
...
if [ ! -d "${OUTDIR}/linux-stable" ]; then
    ...
    echo clone ${KERNEL_REPO} --depth 1 --single-branch --branch ${KERNEL_VERSION}
...
mkdir -p rootfs
cd rootfs
mkdir -p bin dev etc home lib lib64 proc sys sbin tmp usr/bin usr/sbin usr/lib var/log
...
cd ${FINDER_APP_DIR}
make clean
make writer CROSS_COMPILE=${CROSS_COMPILE}gcc
